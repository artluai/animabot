...
// ── Public routes (censored, visibility-gated) ────────────
app.get("/public/status", async (req, res) => {
  ...
  const logs = vis.live_log
    ? await Promise.all(botStatus.logs.slice(-30).map(async l => ({ ...l, msg: await censor(l.msg) })))
    : [];
...
app.get("/public/personality", async (req, res) => {
  ...
  result.emotional_range = p.emotional_range;
  result.memory_depth = p.memory_depth;
  result.memory_bias = p.memory_bias;
  res.json(result);
});
...
app.get("/public/significant", async (req, res) => {
  ...
  const censored = await Promise.all(rows.map(async r => ({
    ...r,
    sender: "******",
    user_message: await censor(r.user_message),
    bot_reply: await censor(r.bot_reply),
    reason: await censor(r.reason),
  })));
  res.json(censored);
});
...
app.get("/public/memory", async (req, res) => {
  ...
  const censored = await Promise.all(rows.map(async r => ({
    ...r,
    session_id: r.session_id.startsWith("!") ? "[private room]" : r.session_id,
    content: await censor(r.content),
  })));
...
app.get("/admin/personality", requireAdmin, async (req, res) => {
  ...
  res.json({
    ...
    public_visibility: parseVisibility(p.public_visibility),
  });
});
...
app.post("/admin/personality", requireAdmin, async (req, res) => {
  ...
  await db.query(
    ...
    [
      system_prompt,
      JSON.stringify(emotional_range),
      ego_notes,
      memory_depth,
      memory_bias,
      JSON.stringify(rules || []),
      JSON.stringify(public_visibility || DEFAULT_VISIBILITY),
    ]
  );
